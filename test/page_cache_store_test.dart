...
void main() {
  test(
    'page cache keeps entries readable across soft ttl and refreshes validation',
    () async {
  ...
  test(
    'page cache read updates memory without rewriting the cache file',
    () async {
      final Directory tempDir = await Directory.systemTemp.createTemp(
        'easy_copy_cache_read_only',
      );
      addTearDown(() => tempDir.delete(recursive: true));
      ...
      final File cacheFile = File(
        '${tempDir.path}${Platform.pathSeparator}page_cache.json',
      );
      final String persistedBeforeRead = await cacheFile.readAsString();

      now = now.add(const Duration(minutes: 1));
      final CachedPageEnvelope? entry = await store.read(
        '/',
        authScope: 'guest',
      );

      expect(entry, isNotNull);
      expect(await cacheFile.readAsString(), persistedBeforeRead);
    },
  );

  test(
    'page cache isolates authenticated entries from guest entries',
    () async {
      final Directory tempDir = await Directory.systemTemp.createTemp(
