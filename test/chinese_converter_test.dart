...
void main() {
  test('关闭模式不会加载 OpenCC 数据或创建引擎', () async {
    int loaderCalls = 0;
    int factoryCalls = 0;
  ...
  test('并发启用共享一次初始化且加载期间显示原文', () async {
    final Completer<String> dataCompleter = Completer<String>();
  ...
  test('快速关闭后迟到的启用结果不能覆盖最新模式', () async {
    ...
    int factoryCalls = 0;
    final ChineseConverter converter = ChineseConverter(
      dataLoader: () => dataCompleter.future,
      engineFactory: (String dataDir) {
        factoryCalls += 1;
        return _FakeEngine();
      },
  ...
  test('模式就绪和关闭时通知独立显示层重建', () async {
    ...
    final ChineseConverter converter = ChineseConverter(
      dataLoader: () => dataCompleter.future,
      engineFactory: (String dataDir) => _FakeEngine(),
    );
    addTearDown(converter.dispose);
    int notifications = 0;
